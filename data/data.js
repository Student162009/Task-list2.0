...
const readUserData = () => {
  if (UserData.length === 0) {
    if (
      !fs.existsSync(UserLOGFilePath) ||
      fs.readFileSync(UserLOGFilePath).length === 0
    ) {
      UserData = [];
    } else {
      try {
...
const writeData = (DATA) => {
  if (Id == "") {
    fs.writeFileSync(
      path.join(__dirname, `json/userTasks/${LOG}.json`),
      JSON.stringify(DATA, null, 2)
    );
  } else {
    fs.writeFileSync(
      path.join(__dirname, `json/MultiTasks/${Id}.json`),
      JSON.stringify(DATA, null, 2)
    );
  }
};
...
const readData = () => {
  if (Id == "") {
    if (Data.length === 0) {
      if (
        !fs.existsSync(path.join(__dirname, `json/userTasks/${LOG}.json`)) ||
        fs.readFileSync(path.join(__dirname, `json/userTasks/${LOG}.json`))
          .length === 0
      ) {
        Data = [];
      } else {
    ...
    if (Data.length === 0) {
      if (
        !fs.existsSync(path.join(__dirname, `json/MultiTasks/${Id}.json`)) ||
        fs.readFileSync(path.join(__dirname, `json/MultiTasks/${Id}.json`))
          .length === 0
      ) {
        Data = [];
      } else {
...
const editPassword = (Login, Data, NewPassword) => {
  if (Data.includes("+375")) {
    const LoginIndex = UserData.findIndex(
      (l) => l.login === Login && l.numtel === Data
    );
    if (LoginIndex === -1) {
      return null;
    }
...
const editLogin = (Login, password, NewLogin) => {
  const LoginIndex = UserData.findIndex(
    (l) => l.login === Login && l.parol === password
  );
  if (LoginIndex === -1) {
    return null;
  }
...
const editFI = (Login, password, NewName, NewFirstName) => {
  const LoginIndex = UserData.findIndex(
    (l) => l.login === Login && l.parol === password
  );
  if (LoginIndex === -1) {
...
const deleteUserData = (Login, password) => {
...
const getDead = () => {
  ...
  const result = Data.filter(
    (task) => task.deadline && new Date(task.deadline) < currentDate
  );
  return result;
};
