  ...
  document
    ...
    .addEventListener("click", function () {
      ...
      if (RepetPassword.type === "password") {
        ...
        icon.classList.add("fa-eye-slash");
        alert(
          "Теперь вы можете увидеть повтор пароля! Убедитесь, что он совпадает с первым, или вы будете в беде!"
        );
      } else {
        ...
        icon.classList.add("fa-eye");
        alert(
          "Повтор пароля снова в безопасности! Теперь вы можете спокойно обновлять пароль!"
        );
      }
    });
  ...
  button.addEventListener("click", async (e) => {
    ...
    if (Newpass == Repetpassword) {
      ...
      if (response.ok) {
        ...
        alert(
          "Пароль не обновлен! Похоже, что наши сервера обнаружили спящего ежа. Попробуйте еще раз!"
        );
      }
    } else {
      alert(
